...
contract MultiAssetMultiPartyCoinTransferInterpreter is
    MultisigTransfer,
    Interpreter
{
    ...
    function interpretOutcomeAndExecuteEffect(
        ...
        MultiAssetMultiPartyCoinTransferInterpreterParams memory params = abi
            .decode(
            encodedParams,
        ...
        for (uint256 i = 0; i < coinTransferListOfLists.length; i++) {
            ...
            uint256 limitRemaining = params.limit[i];
                ...
                LibOutcome.CoinTransfer[] memory coinTransferList
             = coinTransferListOfLists[i];

            for (uint256 j = 0; j < coinTransferList.length; j++) {

                    LibOutcome.CoinTransfer memory coinTransfer
                 = coinTransferList[j];
