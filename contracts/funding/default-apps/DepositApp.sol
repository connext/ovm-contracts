...
contract DepositApp is CounterfactualApp {
        ...
        returns (bytes memory)
    {
        ...
        if (isDeployed(state.multisigAddress)) {
            endingTotalAmountWithdrawn = MinimumViableMultisig(
                state
                    .multisigAddress
            )
        ...
        uint256 endingMultisigBalance = ERC20(state.assetId).balanceOf(
                    ...
                    [
                        LibOutcome.CoinTransfer(
                            ...
                            (endingMultisigBalance -
                                state.startingMultisigBalance) +
                                (endingTotalAmountWithdrawn -
                                    state.startingTotalAmountWithdrawn)
                        ),
                        LibOutcome.CoinTransfer(
