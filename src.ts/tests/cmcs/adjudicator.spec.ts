...
describe("CMCAdjudicator.sol", async function () {
  ...
  const verifyChannelDispute = async (
    ...
    expect(dispute.consensusExpiry).to.be.eq(
      BigNumber.from(ccs.timeout).add(timestamp)
    );
    expect(dispute.defundExpiry).to.be.eq(
      BigNumber.from(ccs.timeout).mul(2).add(timestamp)
    );
    await Promise.all(
      ccs.assetIds.map(async (assetId: string, idx: number) => {
        const defundNonce = await channel.getDefundNonce(assetId);
        expect(defundNonce).to.be.eq(
          BigNumber.from(ccs.defundNonces[idx]).sub(1)
        );
      })
    );
  };

  const verifyTransferDispute = async (
    cts: FullTransferState,
    disputeBlockNumber: number
  ) => {
    const { timestamp } = await provider.getBlock(disputeBlockNumber);
    const transferDispute = await channel.getTransferDispute(cts.transferId);
    expect(transferDispute.transferStateHash).to.be.eq(
      hashCoreTransferState(cts)
    );
    expect(transferDispute.isDefunded).to.be.false;
    expect(transferDispute.transferDisputeExpiry).to.be.eq(
      BigNumber.from(timestamp).add(cts.transferTimeout)
    );
  };
  ...
  const fundChannel = async (ccs: FullChannelState = channelState) => {
    for (const assetId of ccs.assetIds) {
      ...
      if (!depositsA.isZero()) {
        const aliceTx = await channel
          .connect(alice)
          .depositAlice(assetId, depositsA);
        await aliceTx.wait();
      }
    }
  };
  ...
  const disputeChannel = async (ccs: FullChannelState = channelState) => {
    ...
    expect(currBlock.timestamp).to.be.at.least(
      BigNumber.from(timestamp).add(ccs.timeout)
    );
  ...
  const defundChannelAndVerify = async (
    ...
    for (let i = 0; i < assetIds.length; i++) {
      ...
      if ((await channel.getExitableAmount(assetId, alice.address)).gt(Zero)) {
        await (
          await channel.exit(assetId, alice.address, alice.address)
        ).wait();
      }
  ...
  describe("disputeChannel", () => {
    ...
    it("should fail if state.bob is incorrect", async function () {
      await expect(
        channel.disputeChannel(
          { ...channelState, bob: getRandomAddress() },
          aliceSignature,
          bobSignature
        )
      ).revertedWith("CMCAdjudicator: INVALID_CHANNEL");
    ...
    it("should fail if state.channelAddress is incorrect", async function () {
      await expect(
        channel.disputeChannel(
          { ...channelState, channelAddress: getRandomAddress() },
          aliceSignature,
          bobSignature
        )
      ).revertedWith("CMCAdjudicator: INVALID_CHANNEL");
    ...
    it("should fail if alices signature is invalid", async function () {
      await expect(
        channel.disputeChannel(
          channelState,
          await aliceSigner.signMessage(getRandomBytes32()),
          bobSignature
        )
    ...
    it("should fail if channel is not in consensus phase", async function () {
      ...
      await advanceBlocktime(
        BigNumber.from(shortTimeout.timeout).toNumber() + 1
      );
    ...
    it("should fail if nonce is lte stored nonce", async () => {
      ...
      await expect(
        channel.disputeChannel(channelState, aliceSignature, bobSignature)
      ).revertedWith("CMCAdjudicator: INVALID_NONCE");
    });
  ...
  describe("defundChannel", () => {
    ...
    it("should fail if it is not in the defund phase", async function () {
      ...
      await verifyChannelDispute(channelState, blockNumber);
      await expect(
        channel.defundChannel(channelState, channelState.assetIds, [])
      ).revertedWith("CMCAdjudicator: INVALID_PHASE");
    });
    ...
    it("should fail if defund nonce does not increment", async function () {
      ...
      const toDispute = {
        ...channelState,
        defundNonces: channelState.assetIds.map(() => "0"),
      };
      await disputeChannel(toDispute);
      await expect(
        channel.defundChannel(toDispute, toDispute.assetIds, [])
      ).revertedWith("CMCAdjudicator: CHANNEL_ALREADY_DEFUNDED");
    });
  ...
  describe("disputeTransfer", () => {
    ...
    it("should fail if channel is not in defund phase", async function () {
      ...
      await tx.wait();
      await expect(
        channel.disputeTransfer(transferState, getMerkleProof())
      ).revertedWith("CMCAdjudicator: INVALID_PHASE");
    });
    ...
    it("should fail if transfer has already been disputed", async function () {
      ...
      await expect(
        channel.disputeTransfer(transferState, getMerkleProof())
      ).revertedWith("CMCAdjudicator: TRANSFER_ALREADY_DISPUTED");
    });
  ...
  describe("defundTransfer", () => {
    ...
    it("should correctly resolve + defund transfer if transfer is still in dispute (cancelling resolve)", async function () {
      ...
      await prepTransferForDefund();
      const preDefundAlice = await getOnchainBalance(
      ...
      await (
        await channel.exit(transferState.assetId, alice.address, alice.address)
      ).wait();
      expect(
        await getOnchainBalance(transferState.assetId, alice.address)
      ).to.be.eq(preDefundAlice.add(transferState.balance.amount[0]));
      expect(
        await getOnchainBalance(
          transferState.assetId,
    ...
    it("should correctly resolve + defund transfer if transfer is still in dispute (successful resolve)", async function () {
      ...
      expect(
        await getOnchainBalance(transferState.assetId, alice.address)
      ).to.be.eq(preDefundAlice);
      expect(
        await getOnchainBalance(
          transferState.assetId,
          transferState.balance.to[1]
        )
      ).to.be.eq(transferState.balance.amount[0]);
    });
        ...
        await channel
          ...
          .defundTransfer(
            ...
            await signChannelMessage(
              transferState.initialStateHash,
              bob.privateKey
            )
          )
      ).wait();
    ...
    it("should correctly defund transfer when transfer is not in dispute phase", async function () {
      ...
      await advanceBlocktime(
        BigNumber.from(transferState.transferTimeout).toNumber()
      );
      ...
      const postDefundAlice = await getOnchainBalance(
        transferState.assetId,
        alice.address
      );
      expect(postDefundAlice).to.be.eq(
