...
describe("CMCAsset", function () {
  ...
  beforeEach(async () => {
    assetTransfer = await getOvmContract("CMCAsset", alice);
    channel = await createOvmTestChannel();
    ...
    nonconformingToken = await getOvmContract("NonconformingToken", alice);
  ...
  describe("getTotalTransferred", () => {
    it("should fail if called directly", async () => {
      await expect(assetTransfer.getTotalTransferred(AddressZero)).revertedWith(
        "Mastercopy: ONLY_VIA_PROXY"
      );
    });

    it("should work when nothing has been transferred", async () => {
      expect(await channel.getTotalTransferred(AddressZero)).to.be.eq(
        BigNumber.from(0)
      );
      expect(await channel.getTotalTransferred(token.address)).to.be.eq(
        BigNumber.from(0)
      );
    });
  });
  ...
  describe("getExitableAmount", () => {
    it("should fail if called directly", async () => {
      await expect(
        assetTransfer.getExitableAmount(AddressZero, bob.address)
      ).revertedWith("Mastercopy: ONLY_VIA_PROXY");
    });

    it("should work when nothing has been transferred", async () => {
      expect(
        await channel.getExitableAmount(AddressZero, bob.address)
      ).to.be.eq(BigNumber.from(0));
      expect(
        await channel.getExitableAmount(token.address, bob.address)
      ).to.be.eq(BigNumber.from(0));
    });
  });
  ...
  describe("makeExitable", () => {
    ...
    it.skip("should work for ETH transfers", async () => {
    ...
    it("should work for ERC20 token that does not return `bool` from transfer", async () => {
      ...
      await channel.testMakeExitable(
        nonconformingToken.address,
        bob.address,
        value
      );
      expect(await nonconformingToken.balanceOf(bob.address)).to.be.eq(
        preTransfer
      );
      expect(
        await channel.getTotalTransferred(nonconformingToken.address)
      ).to.be.eq(Zero);
      expect(
        await channel.getExitableAmount(nonconformingToken.address, bob.address)
      ).to.be.eq(value);
    });
  });
  ...
  describe("makeBalanceExitable", () => {
    beforeEach(async () => {
      ...
      await (
        await token
          .connect(alice)
          .transfer(channel.address, BigNumber.from(10000))
      ).wait();
    });
    ...
    it("should work", async () => {
      ...
      await channel.testMakeBalanceExitable(token.address, balance);
      ...
      expect(
        await channel.getExitableAmount(token.address, rando.address)
      ).to.be.eq(valueRando);
    });
  });
  ...
  describe("exit", () => {
    ...
    beforeEach(async () => {
      ...
      // await (
      //   await bob.sendTransaction({ to: channel.address, value: fund })
      // ).wait();
      await (
        await failingToken
          .connect(bob)
          .succeedingTransfer(channel.address, fund)
      ).wait();

      // Make failing transfer
      const preTransfer = await failingToken.balanceOf(bob.address);
      await (
        await channel.testMakeExitable(failingToken.address, bob.address, value)
      ).wait();
      expect(await failingToken.balanceOf(bob.address)).to.be.eq(preTransfer);
      expect(await channel.getTotalTransferred(failingToken.address)).to.be.eq(
        BigNumber.from(0)
      );
      expect(
        await channel.getExitableAmount(failingToken.address, bob.address)
      ).to.be.eq(value);
    ...
    it("should fail if owner is not msg.sender or recipient", async () => {
      await expect(
        channel
          .connect(rando)
          .exit(failingToken.address, bob.address, rando.address)
      ).revertedWith("CMCAsset: OWNER_MISMATCH");
    });

    it("should fail if withdrawable amount is 0", async () => {
      await expect(
        channel.connect(bob).exit(token.address, bob.address, bob.address)
      ).revertedWith("CMCAsset: NO_OP");
    });
    ...
    it("should fail if transfer fails", async () => {
      ...
      await expect(
        channel
          .connect(bob)
          .exit(failingToken.address, bob.address, bob.address)
      ).revertedWith("CMCAsset: TRANSFER_FAILED");
    });
    ...
    it("should allow ERC20 token to be withdrawable if transfer fails", async () => {
      const preTransfer = await failingToken.balanceOf(bob.address);
      await (
        await channel.exit(failingToken.address, bob.address, bob.address)
      ).wait();
      expect(await failingToken.balanceOf(bob.address)).to.be.eq(
        preTransfer.add(value)
      );
    });
  });
});
