...
describe("ChannelMastercopy", function () {
  ...
  beforeEach(async () => {
    mastercopy = await getOvmContract("ChannelMastercopy", alice);
  });
  ...
  it("all public methods should revert bc it's the mastercopy", async () => {
    ...
    for (const method of [
      ...
      {
        name: "disputeChannel",
        args: [CoreChannelStateZero, HashZero, HashZero],
      },
      {
        name: "defundChannel",
        args: [CoreChannelStateZero, [AddressZero], [Zero]],
      },
      { name: "disputeTransfer", args: [CoreTransferStateZero, []] },
      {
        name: "defundTransfer",
        args: [CoreTransferStateZero, HashZero, HashZero, HashZero],
      },
    ]) {
      await expect(mastercopy[method.name](...method.args)).to.be.revertedWith(
