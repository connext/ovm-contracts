...
const encodeAppState = (
  state: SimpleSwapAppState,
  onlyCoinTransfers: boolean = false
): string => {
  if (!onlyCoinTransfers)
    return defaultAbiCoder.encode([swapAppStateEncoding], [state]);
...
describe("SimpleTwoPartySwapApp", () => {
  ...
  describe("update state", () => {
    it("can compute outcome with update", async () => {
      ...
      expect(decoded[0][0].amount.toString()).eq(
        state.coinTransfers[0][0].amount.toString()
      );
      expect(decoded[1][0].to).eq(state.coinTransfers[1][0].to);
      expect(decoded[1][0].amount.toString()).eq(
        state.coinTransfers[1][0].amount.toString()
      );
    });
  });
});
