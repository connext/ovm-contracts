...
const encodeAppState = (
  ...
  if (!onlyCoinTransfers)
    return defaultAbiCoder.encode([linkedTransferAppStateEncoding], [state]);
...
describe("SimpleLinkedTransferApp", () => {
  ...
  async function computeOutcome(
    ...
    return simpleLinkedTransferApp.functions.computeOutcome(
  ...
  describe("update state", () => {
    it("can redeem a payment with correct hash", async () => {
      ...
      const linkedHash = createLinkedHash(
        transferAmount,
        assetId,
        paymentId,
        preImage
